...
class EvalResult:
    def __init__(self, num_avail_premises: int, 
                 hits_on: list[int]) -> None:
        assert type(num_avail_premises) == int
...
class Evaluator:
    def __init__(self,
                 ...
                 split: str) -> None:
        assert isinstance(model_wrapper, PremiseModelWrapper)
        ...
        for raw_dataset_file in tqdm(os.listdir(self.split_loc)):
            ...
            for proof in parsed_dataset_file.proofs:
                for step in proof.steps:
                    num_steps += 1
                    filter_result = self.model_wrapper.premise_filter.get_pos_and_avail_premises(
                    ...
                    if (num_positive_premises == 0) or (num_avail_premises == 0) :
                        continue
                    ranked_premises_generator = self.model_wrapper.get_ranked_premise_generator(
...
if __name__=="__main__":
    ...
    parser.add_argument("partitioned_data_loc",
                        help="Location of partioned raw data.")
