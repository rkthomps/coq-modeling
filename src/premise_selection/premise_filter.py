...
class FilteredResult:
    def __init__(self, pos_premises: list[Sentence], 
                 avail_premises: list[Sentence]) -> None:
...
class PremiseFilter:
    def __init__(self, 
                 ...
                 general_excludes: list[TermType]=[]) -> None:
    ...
    def __print_warnings(self) -> None:
        if len(self.coq_excludes) > 0:
            print((f"Excluding term types {self.coq_excludes} for premise selection "
                  "if they come from the coq standard library."))
        if len(self.non_coq_excludes) > 0:
            print(f"Excluding term types {self.non_coq_excludes} for premise selection "
    ...
    def get_pos_and_avail_premises(self, step: FocusedStep, proof: Proof, 
                                   dset_obj: DatasetFile) -> FilteredResult:
        ...
        for pos_premise in all_positive_candidates:
            ...
            prev_line_in_file = same_file and (pos_premise.line < proof.theorem.term.line)
            ...
            if not premise_available:
                print(f"Same file positive premise not available at {pos_premise.file_path}:{pos_premise.line}",
                      file=sys.stderr)
            if not premise_in_context:
                print(f"Positive premise not in context at {pos_premise.file_path}:{pos_premise.line}",
                      file=sys.stderr)
