...
def mine_coq_file_goals(
    coq_file: CoqFile, file_info: FileInfo, split: Split
) -> list[GoalTermRecord]:
    ...
    for proof_index in proof_indices:
        ...
        assert len(subproofs) == len(orig_steps)
        try:
            prev_pretty_goal = ""
            for i, step in enumerate(orig_steps[:-1]):
                subproof = subproofs[i]
                if subproof is None:
                    coq_file.add_step(proof_index + i, step)
                    continue
                go_through_point(coq_file, proof_index + i)
                try:
                    cur_goal = coq_file.current_goals
                    fg_goal = get_fg_goal(cur_goal)
                    pretty_goal = repr(fg_goal)
                    norm_goal = get_norm_goal(coq_file, proof_index + i)
                    proof_start_idx = proof_index + i + 1
                    ...
                    if pretty_goal == prev_pretty_goal:
                        records[-1] = record
                    else:
                        records.append(record)
                    prev_pretty_goal = pretty_goal
...
def mine_file_goals(
    ...
    try:
        with CoqFile(temp_loc, workspace=workspace_loc) as coq_file:
            try:
                files_records = mine_coq_file_goals(coq_file, file_info, split)
