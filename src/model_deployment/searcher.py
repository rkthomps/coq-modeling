...
class ProofSearchTree:
    ...
    def from_json(cls, json_data: Any) -> ProofSearchTree:
        ...
        return cls(valid, final_tactic, makes_progress, valid_proof_steps,
                   combined_proof_steps, model_tactic, combined_model_tactics, goal, score, expanded, children)
...
class SearchTreeManager:
    def __init__(self, 
        ...
        self.search_tree = ProofSearchTree(
            initial_validity, final_tactic, makes_progress, initial_valid_steps, 
            combined_valid_steps, initial_tactic, initial_combined_tactics, initial_goal, initial_score)
        self.frontier: list[ProofSearchTree] = []
        self.seen_goals: list[NodeGoal] = [node_goal]
        heapq.heappush(self.frontier, self.search_tree)


    def __get_request_contents(self, combined_proof_steps: list[str], 
                               target_combined_steps: str) -> LmExample:
        return self.proof_manager.get_example(combined_proof_steps, target_combined_steps)
    ...
    def __get_complete_child_node(self, proof_check_result: ProofCheckResult, 
        ...
        complete_node = ProofSearchTree(
            valid, final_tactic, makes_progress, 
            valid_steps, combined_steps,
            attempted_tactic, combined_tactics,
            pretty_goal, parent_node.score.agg(score)
        )
        return complete_node


    def __get_invalid_child_node(self, proof_check_result: ProofCheckResult,
                                 attempted_tactic: str,
                                 parent_node: ProofSearchTree,
                                 score: NodeScore) -> ProofSearchTree:
        assert proof_check_result.current_goals is None
        ...
        invalid_node = ProofSearchTree(
            valid, final_tactic, makes_progress, invalid_steps, combined_steps,
            attempted_tactic, combined_tactics, pretty_goal, parent_node.score.agg(score)) 
        return invalid_node


    def __get_valid_child_node(self, proof_check_result: ProofCheckResult, 
                               attempted_tactic: str,
                               parent_node: ProofSearchTree,
                               score: NodeScore) -> tuple[ProofSearchTree, NodeGoal]:
        ...
        makes_progress = (
            goal_progress or 
            self.__is_bullet(tactic_str) or 
            self.__is_first_proof_tactic(parent_node.combined_proof_str(), tactic_str))
        valid = True
        final_tactic = False
        new_leaf = ProofSearchTree(
            valid, final_tactic, makes_progress, valid_proof_steps, 
            combined_proof_steps, attempted_tactic, combined_tactics,  
            pretty_goal, parent_node.score.agg(score))
            ...
            match proof_check_result.tactic_result:
                ...
                case TacticResult.VALID:
                    ...
                    if valid_node.makes_progress:
                        ...
                        clean_steps = ProofSearchTree.clean_tactic(valid_node.steps_proof_str())
                        clean_orig_step = ProofSearchTree.clean_tactic(tactic) 
                        print(f"adding new child node {clean_steps} from {clean_orig_step}")
        leaf_subtree.children = children
        return None 
