...
class ClassicalSearcherConf:
    ...
    @classmethod
    def from_yaml(cls, yaml_data: Any) -> ClassicalSearcherConf:
        return cls(
...
class ClassicalSearcher:
    ...
    def search(self) -> ClassicalSuccess | ClassicalFailure:
        start = time.time()
        for i in range(self.max_search_steps):
            cur = time.time()
            if self.timeout <= cur - start:
                return ClassicalFailure(
                    cur - start, self.total_model_time, self.root_candidate
                )
            if len(self.frontier) == 0:
                return ClassicalFailure(
                    cur - start, self.total_model_time, self.root_candidate
                )
            possible_success = self.search_step()
        ...
        return ClassicalFailure(
            time.time() - start, self.total_model_time, self.root_candidate
        )
    ...
    def search_step(self) -> Optional[Candidate]:
