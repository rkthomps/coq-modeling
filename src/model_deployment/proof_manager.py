...
class ProofCheckResult:
    def __init__(self, tactic_result: TacticResult, 
                 valid_steps: list[str],
                 current_goals: Optional[GoalAnswer]) -> None:
...
class ProofManager:
    ...
    def __init__(self, file_path: str,
                 ...
                 aux_file_path: str,
                 lm_example_config: LmExampleConfig) -> None:
    ...
    def set_proof_file(self, valid_steps: list[str], 
                       target_combined_proof: str,
                       partial_proof_suffix: str) -> None:
    ...
    def check_proof(self, partial_proof: str, new_tactic: str, prev_valid_steps: list[str]) -> ProofCheckResult:
        ...
        with CoqFile(self.aux_file_path) as coq_file:
            ...
            if not coq_file.in_proof:
                return ProofCheckResult(TacticResult.COMPLETE, new_valid_steps, current_goals) 
            return ProofCheckResult(TacticResult.VALID, new_valid_steps, current_goals) 


    def get_example(self, valid_steps: list[str], target_combined_steps: str) -> LmExample:
        ...
        dataset_obj = self.get_dataset_file(valid_steps, target_combined_steps, partial_proof_suffix) 
        examples = self.example_type.from_dataset_file(dataset_obj, self.premise_wrapper, partial_proof_suffix)
        example = examples[-1]
        return example
